...
def make_dataset(constants: PhysicsConstants, params: SimulationParameters, dataset_params: DatasetParameters, dim: bool = True):
    ...
    for d_val_m in tqdm(dataset_params.d_val_m, desc="d_val_m"):
        for k_rel in dataset_params.k_rel:
            ...
            if dim:
                ...
                alpha = alpha_from_dim(C)
            r, t_out, CN, CF, CI = forward_solver(PN, DN, alpha, C, params)
    ...
    if dataset_params.save:
        ...
        np.savez(filename, r=r_vals, t_out=t_out_vals, CN=CN_vals, CF=CF_vals, CI=CI_vals, d_val_m=d_vals, k_rel=k_rel_vals)
    else:      
        return { "r": r_vals, "t_out": t_out_vals, "CN": CN_vals, "CF": CF_vals, "CI": CI_vals, "d_val_m": d_vals, "k_rel": k_rel_vals}
