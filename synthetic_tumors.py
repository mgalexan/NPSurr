...
for q in q_vals:
    for P0 in P0_vals:
        for d_pore, xi_ECM in zip(d_pore_vals, xi_ECM_vals):
            phys.append(PhysicsConstants(P0= P0, d_pore=d_pore, xi_ECM=xi_ECM, q= q))
            dataset.append(DatasetParameters(filename= f"P0={P0:.2g}_d_pore={d_pore:.2g}_q={q}_dataset.npz"))
            ml.append(MLParameters(data_filepath= f"/u/mgalexan/NPSurr/data/P0={P0:.2g}_d_pore={d_pore:.2g}_q={q}_dataset.npz", save_path=f"/u/mgalexan/NPSurr/data/P0={P0:.2g}_d_pore={d_pore:.2g}_q={q}_model.pt"))
